...
def dense_sum_trapezoid(F, P, out_c=3):
    return parallel(identity(), identity(), downscale().map_on_lists()).then( #Very first thing, downsize the 4x to 2x
           ...
           replicate_over_list(lambda: add_bias(F).then_apply(activ), 3)).then( #bias each one, and run through activation
...
def zero_pad_edges(x):
    return zero_pad_half(zero_pad_two_max(x))
...
def scale_conv_layer(trapezoid):
    return to_neural_op(zero_pad_edges).then_apply(consecutive_triples).then(
...
def dense_scale_conv_layer(dense_trapezoid):
    ...
    return to_neural_op(zero_pad_edges).then_apply(consecutive_triples).map_on_lists().then_apply(
